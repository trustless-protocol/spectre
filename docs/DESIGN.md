...
- `modules` array, each entry has `name`, `src_chain`, `dst_chain`, and `config`:
  - `cosmos_to_eth` config: tm_rpc_url, ics26_address, eth_rpc_url, ics07_client, wrapper_verifier, membership, misbehaviour, update_client, and optional fields: fetch_timeout, trusting_period, trust_level, proof_type. **One entry per Cosmos source** (distinct `ics26_client_id`); `start` runs an independent relay loop for each, and `create-clients --source <ics26_client_id>` targets one.
