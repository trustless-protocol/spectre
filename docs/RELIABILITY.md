...
- `SubscribeL2` performs startup log recovery for OP/L2 `SendPacket` and `WriteAcknowledgement` events. `L2_STARTUP_LOOKBACK_BLOCKS` defaults to 256; set it large enough to cover the expected attestation/finality wait and relayer downtime. Set it to `0` to start at the current L2 head and disable startup recovery. This is a temporary operational escape hatch until the recovery window can be derived from each chain's block time and configured finality policy.
...
- **Circuit update**: Run `scripts/solidity/build-prover-artifacts.sh --force` to intentionally regenerate the paired configured artifacts and diagnostic provenance; redeploy every regenerated `Groth16Verifier_N*.sol` and re-register its bucket via `SignatureVerifier.setBucket(...)` before the next E2E run
