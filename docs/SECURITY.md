...
- The relayer's set of buckets (`prover.Buckets`) caps the maximum number of distinct active signers; chains with quorum sets larger than the biggest bucket cannot be served until a larger bucket is added and deployed.
  **Concretely for Cosmos Hub** (`LC-02`): `ValidatorSetLib.MAX_VALIDATOR_COUNT = 180`
  (`contracts/light-clients/store/ValidatorSetLib.sol:16`) exactly matches Cosmos Hub's current
  validator count. Quorum requires >2/3 of pinned voting power — under an equal-power assumption
  that's `floor(2*180/3)+1 = 121` unique signers, just inside the largest configured bucket
  (N=128, `relayer/prover/buckets.go`), leaving only ~7 validators of headroom. That headroom is a
  best case: under a skewed power distribution where the largest stake-holders are offline, quorum
  can require signatures from far more than 121 of the long-tail validators, eating the headroom
  faster than the equal-power estimate suggests. There is no on-chain oracle of the counterparty's
  live validator count, so this is a hard liveness ceiling that cannot be enforced or asserted
  on-chain — only monitored for off-chain. **Operational doctrine**: any Cosmos Hub governance
  proposal to raise its validator count is a protocol-upgrade trigger for this project (requires
  provisioning a larger Groth16 bucket — circuit regen + redeploy of every `Groth16Verifier_N{N}` +
  `setBucket` — before `MAX_VALIDATOR_COUNT` can be safely raised); Cosmos Hub's live validator
  count nearing 180 must be monitored off-chain (no in-repo alerting mechanism exists yet to
  automate this — same gap noted for `RLY-01`/`RLY-02`).
