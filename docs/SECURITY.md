...
- The checked in-repo prover manifest currently enables N4 only, while the production deployment compatibility surface preserves N ∈ {4, 8, 16, 32, 64, 128}. Any production launch must enable the required buckets in the relayer and publish, deploy, and register each matching artifact set; a header whose quorum exceeds the largest actually provisioned bucket cannot be served. Historical benchmark reports are measurements, not evidence that a paired artifact set is deployed.
