...
# Local E2E scenario: Multiple ETH→Cosmos packets timeout, verify full refund.
#
# Corresponds to: Test_5_TimeoutPacketFromEth, Test_10_TimeoutPacketFromEth
...
# ─── Send N transfers with short timeout ───
TIMEOUT_SECONDS="${TIMEOUT_SECONDS:-15}"
TIMEOUT=$(($(date +%s) + TIMEOUT_SECONDS))
log_header "Sending ${BATCH_SIZE} ICS20Transfers (${TIMEOUT_SECONDS}s timeout)"
...
log "Multicall submitted (waiting for timeout...)"
...
# ─── Summary ───
log_header "Timeout Batch Test Summary"
log_kv "Direction" "ETH → Cosmos"
log_kv "Batch size" "$BATCH_SIZE"
log_kv "Amount per packet" "${AMOUNT} wei"
log_kv "Total sent" "${TOTAL_AMOUNT} wei"
...
if [ "${REFUNDED:-false}" = "true" ]; then
  ...
  log "  • Cosmos voucher = 0 (packets never relayed)"
