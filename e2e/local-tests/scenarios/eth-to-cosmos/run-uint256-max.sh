...
# Local E2E scenario: Large amount (uint256) edge case transfer ETH→Cosmos.
# Uses StartingERC20Balance / 2 as the transfer amount to stress-test uint256 handling.
#
# Corresponds to: Test_ICS20TransferUint256TokenfromEthereumToCosmosAndBack
...
# ─── Determine large transfer amount ───
log_header "Determining Transfer Amount"
STARTING_BALANCE="$(eth_balance_of "$ERC20_ADDRESS" "$ETH_SENDER")"
log_kv "Starting ERC20 balance" "$STARTING_BALANCE"

# Default to half of starting balance, or a very large fixed amount if query fails
if [ -n "$STARTING_BALANCE" ] && [ "$STARTING_BALANCE" != "0" ]; then
  AMOUNT="$(echo "$STARTING_BALANCE / 2" | bc)"
else
  AMOUNT="${TRANSFER_AMOUNT:-5000000000000000000}"
fi
log_kv "Large transfer amount" "$AMOUNT"

# ─── Record pre-transfer state ───
log_header "Pre-transfer State"
BEFORE_ETH_BALANCE="$STARTING_BALANCE"
...
log_header "Sending Large ICS20Transfer"
log "Approving ERC20 allowance..."
eth_approve "$ERC20_ADDRESS" "$ICS20_ADDRESS" "$AMOUNT"

log "Submitting sendTransfer with large amount..."
...
while [ $poll -lt $MAX_POLLS ]; do
  ...
  if bc_ge "$CURRENT_COSMOS" "$TARGET_COSMOS_BALANCE"; then
    log_ok "[poll ${poll}/${MAX_POLLS}] Cosmos voucher received │ balance=${CURRENT_COSMOS}"
    SUCCESS=true
    break
  fi
...
# ─── Summary ───
log_header "Uint256 Edge Case Test Summary"
log_kv "Direction" "ETH → Cosmos"
log_kv "Transfer amount" "${AMOUNT} wei"

if [ "${SUCCESS:-false}" = "true" ]; then
  log_ok "RESULT: PASS"
  log "  • Large amount transfer submitted without overflow"
  log "  • Cosmos voucher balance increased correctly"
else
  log_err "RESULT: FAIL"
