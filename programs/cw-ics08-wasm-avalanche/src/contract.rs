//! Avalanche C-Chain authenticated ICS-08 Wasm client entrypoints.

#![allow(missing_docs)]

l2_client::l2_client_entrypoints!(l2_client::StaticL2Client<avalanche_cchain::AvalancheProfile>);
...
mod tests {
    ...
    use avalanche_cchain::AvalancheProfile;
    use l2_client::{
        error::Error, msg::MigrateMsg, state::RuntimeProfile, L2LightClient, StaticL2Client,
    };

    #[test]
    fn composes_the_shared_kernel_with_the_avalanche_profile() {
        fn assert_composition<Client: L2LightClient<Profile = AvalancheProfile>>() {}
        assert_composition::<StaticL2Client<AvalancheProfile>>();
        assert_eq!(
            AvalancheProfile::expected_profile_version(),
    ...
    fn rejects_a_profile_for_a_different_artifact() {
        let mut profile: serde_json::Value = serde_json::from_str(include_str!(
            "../../../packages/avalanche-cchain/config/fuji.json"
        ))
        .unwrap();
