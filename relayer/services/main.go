...
func (s *Services) handleCosmos(ctx Context, batch CosmosBatch) {
	...
	for _, packet := range batch.Packets {
		switch packet.Type {
		case CosmosSend:
			if cosmosPacketExpiredOnEth(packet, ethBlockTime) {
				log.Printf("[RecvPacket] seq=%d timed out before ETH relay (timeout=%d <= eth_block_time=%d), submitting MsgTimeout to Cosmos",
