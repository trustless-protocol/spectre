...
func TestRecoveryStateMissingFileAndPartialCheckpoints(t *testing.T) {
	...
	if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: 20, EthSendBlock: 30}); err != nil {
		t.Fatal(err)
	}
	if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: 10, EthWriteAckBlk: 40}); err != nil {
		t.Fatal(err)
	}
	got, ok := store.Get("source-a")
	if !ok || got != (RecoveryCursors{CosmosHeight: 10, EthSendBlock: 30, EthWriteAckBlk: 40}) {
	...
	if !ok || got.CosmosHeight != 10 || got.EthSendBlock != 30 || got.EthWriteAckBlk != 40 {
		t.Fatalf("reloaded cursors = %+v, ok=%t", got, ok)
	}
}

func TestRecoveryStateCheckpointCanMoveBackward(t *testing.T) {
	resetLoadedRecoveryStoresForTest()
	...
	if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: 100, EthSendBlock: 200}); err != nil {
		t.Fatal(err)
	}
	if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: 50}); err != nil {
		t.Fatal(err)
	}

	got, ok := store.Get("source-a")
	want := RecoveryCursors{CosmosHeight: 50, EthSendBlock: 200}
	if !ok || got != want {
		t.Fatalf("checkpoint cursors = %+v, ok=%t, want %+v", got, ok, want)
	}
	...
	got, ok = reloaded.Get("source-a")
	if !ok || got != want {
		t.Fatalf("reloaded checkpoint cursors = %+v, ok=%t, want %+v", got, ok, want)
	}
}

func TestRecoveryStateFailedWriteDoesNotAdvanceMemory(t *testing.T) {
	resetLoadedRecoveryStoresForTest()
	dir := t.TempDir()
	...
	if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: 10}); err != nil {
		t.Fatal(err)
	}
	...
	if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: 20}); err == nil {
		t.Fatal("Save succeeded when the target path was a directory")
	}
	got, ok := store.Get("source-a")
	if !ok || got.CosmosHeight != 10 {
		t.Fatalf("cursor after failed write = %+v, ok=%t, want height 10", got, ok)
	}
	...
	got, ok = reloaded.Get("source-a")
	if !ok || got.CosmosHeight != 20 {
		t.Fatalf("reloaded cursor after retry = %+v, ok=%t, want height 20", got, ok)
	}
}
...
func TestRecoveryStateMemoizationPreservesConcurrentSources(t *testing.T) {
	...
	for i, id := range []string{"source-a", "source-b", "source-c"} {
		...
		go func(id string, cursor uint64) {
			defer wg.Done()
			if err := a.SaveCheckpoint(id, RecoveryCursors{CosmosHeight: cursor}); err != nil {
...
func TestRecoveryStateWriteLeavesNoTempFiles(t *testing.T) {
	resetLoadedRecoveryStoresForTest()
	dir := t.TempDir()
	store, err := LoadRecoveryState(filepath.Join(dir, "state.json"))
	if err != nil {
		t.Fatal(err)
	}
	for i := range 5 {
		if err := store.SaveCheckpoint("source-a", RecoveryCursors{CosmosHeight: uint64(i + 1)}); err != nil {
	...
	for _, entry := range entries {
		if strings.HasSuffix(entry.Name(), ".tmp") {
			t.Fatalf("temporary recovery file %q was not removed", entry.Name())
		}
	}
}
