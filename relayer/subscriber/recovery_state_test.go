...
func TestSubscriberResumesFromPersistedCursors(t *testing.T) {
	...
	if err := store.Save("source-a", services.RecoveryCursors{CosmosHeight: 100, EthSendBlock: 120, EthWriteAckBlk: 900}); err != nil {
		t.Fatal(err)
	}
...
func TestScanEthRangePersistsOnlySuccessfulChunks(t *testing.T) {
	...
	calls := 0
	persist := func() {
		if err := store.Save("source-a", services.RecoveryCursors{EthSendBlock: cursor}); err != nil {
			t.Fatal(err)
		}
	}
