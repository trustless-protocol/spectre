...
The protected-doc gate checks working-tree, index, and committed `baseline..HEAD` changes. The authoritative `docs/refactor/01-Solidity-Contracts.md` remains unchanged from `origin/refactor/design-contracts@115397d`. A separate review copy at `docs/01-Solidity-Contracts.md` preserves Duc and Dong's comments together with Dzung's responses; it does not replace or modify the protected design source. Four local-only stale derivative design files, which were never present on the design branch or in committed history, were removed from the worktree and from the invalid evidence-digest inventory; runtime, storage, fixture, gas, and generated-binding baseline values remain unchanged.
