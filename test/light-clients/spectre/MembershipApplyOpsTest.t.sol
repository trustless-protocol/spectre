...
contract ApplyOpsHarness is Membership {
    function exposedApplyLeaf(
        MembershipMsgs.LeafOp memory op,
        bytes memory key,
        bytes memory value
    )
        external
    ...
    function exposedApplyInner(MembershipMsgs.InnerOp memory inner, bytes32 child) external view returns (bytes32) {
...
contract MembershipApplyOpsTest is Test {
    ...
    function _leafOp(
        ...
        internal
        pure
        returns (MembershipMsgs.LeafOp memory)
    {
        return MembershipMsgs.LeafOp({
            hashOp: hashOp,
    ...
    function test_prepareLeafData_noHash_nonAligned() public view {
        ...
        bytes memory got = h.exposedPrepareLeafData(MembershipMsgs.HashOp.NO_HASH, data);
    ...
    function test_prepareLeafData_sha256Prehash() public view {
        ...
        bytes memory got = h.exposedPrepareLeafData(MembershipMsgs.HashOp.SHA256, data);
    ...
    function test_applyLeaf_sha256_nonAligned() public view {
        ...
        bytes32 got = h.exposedApplyLeaf(_leafOp(MembershipMsgs.HashOp.SHA256, prefix), key, value);
    ...
    function test_applyLeaf_keccak_nonAligned() public view {
        ...
        bytes32 got = h.exposedApplyLeaf(_leafOp(MembershipMsgs.HashOp.KECCAK256, prefix), key, value);
    ...
    {
        return MembershipMsgs.InnerOp({ hashOp: hashOp, prefix: prefix, suffix: suffix });
    }
    ...
    function test_applyInner_sha256_nonAligned() public view {
        ...
        bytes32 got = h.exposedApplyInner(_innerOp(MembershipMsgs.HashOp.SHA256, prefix, suffix), child);
    ...
    function test_applyInner_keccak_prefixCrossesWordBoundary() public view {
        ...
        bytes32 got = h.exposedApplyInner(_innerOp(MembershipMsgs.HashOp.KECCAK256, prefix, suffix), child);
