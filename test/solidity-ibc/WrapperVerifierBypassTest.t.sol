...
/// @title WrapperVerifierBypassTest
/// @notice Locks down the WrapperVerifier code-less-verifier bypass
...
///             `setBucket`).
contract WrapperVerifierBypassTest is Test {
    /// Harness that exposes the OWNER-gated `setBucket` setter without
    /// requiring an extra deployer actor.
    WrapperVerifierHarness internal wrapper;

    function setUp() public {
        wrapper = new WrapperVerifierHarness(address(this));
    }
    ...
    ///      state variable of `WrapperVerifier` (slot 0); `OWNER` is
